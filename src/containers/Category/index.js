...
class CategoryScreen extends PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (props.filters !== nextProps.filters) {
      ...
      this.pageNumber = 1;
      props.clearProducts();
      props.fetchProductsByCategoryId(null, this.pageNumber++, 100, this.newFilters);
    }
  ...
  render() {
    ...
    return (
      <View style={[styles.container, { backgroundColor: background }]}>
        ...
        {
          this.props.selectedCategory.parent === 173 &&
          <View >
            <Text style={[styles.label2]}>
              {Languages.labeldrink}
            </Text>
          </View>
        }

        {this.renderList(products.list.sort((a, b) => a.menu_order - b.menu_order))}
        <FilterPicker
  ...
  onEndReached = () => {
    ...
    if (!products.isFetching && products.stillFetch) {
      if (this.newFilters) {
        fetchProductsByCategoryId(selectedCategory.id, this.pageNumber++, 100, this.newFilters);
      } else {
        fetchProductsByCategoryId(selectedCategory.id, this.pageNumber++);
      }
    }
