...
export const actions = {
  addCartItem: (dispatch, product, variation, promotion) => {
  ...
  removeCartItem: (dispatch, product, variation, promotion) => {
    dispatch({
    ...
    case types.ADD_CART_ITEM: {
      ...
      state.cartItems.map((cartItem) => {
        if (cartItem.product.id === action.product.id) {
          cartItemTem = cartItem;
        }
      }
      );

      // alert( cartItemTem?  cartItemTem.quantity + 1: 1);
      return Object.assign(
        ...
        {
          total: state.total + 1,
          totalPrice: state.totalPrice + getPriceWithDiscounts(action, cartItemTem),
        }
      );
    }
    case types.REMOVE_CART_ITEM: {
      ...
      return index == -1
        ...
        : Object.assign(
          ...
          {
            total: state.total - 1,
            totalPrice: state.cartItems.length > 0? (state.totalPrice - getPriceWithDiscountsRemove(action,cartItemTem)): 0,
          }
        );
    }
...
function getPriceWithDiscounts(action, product) {
  //alert((product ? (product.quantity + 1) : 1) + "  " + action.promotion);

  let total = false;
  if(action.promotion > 1){
  total = Number.isInteger(((product ? (product.quantity + 1) : 1) / action.promotion));
  }
  return Number(
    action.variation === undefined ||
      action.variation == null ||
      action.variation.price === undefined ||
      action.variation.price === ""
      ? (total ? 0 : Tools.getPriceIncluedTaxAmountWithDiscounts(action.product, null, true))
      : (total ? 0 : Tools.getPriceIncluedTaxAmountWithDiscounts(action.variation, null, true))
  );
}

function getPriceWithDiscountsRemove(action, product) {
 // alert((product ? (product.quantity ) : 1) + "  " + action.promotion);

  let total = false;
  if(action.promotion > 1){
  total = Number.isInteger(((product ? (product.quantity ) : 1) / action.promotion));
  }
  return Number(
    action.variation === undefined ||
      action.variation == null ||
