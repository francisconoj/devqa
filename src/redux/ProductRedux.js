...
export const actions = {
  ...
  fetchProductsById: async (
    ...
    const json = await WooWorker.getProductId(
      productId
    );
    if (json === undefined) {
      ...
      dispatch(actions.fetchProductSuccess(json));
    }
  },
  fetchProductSuccess: (item) => ({
  ...
  getCouponAmount: async (dispatch, code, totalPrice) => {
    ...
    if (json === undefined) {
      ...
      json.forEach((item) => {
        if (item.code.toLowerCase().trim() === code.toLowerCase().trim()) {
          ...
          if (item.date_expires) {
            ...
            if (dateExpires.diff(today) > 0) {
              ...
              if (totalPrice >= parseFloat(item.minimum_amount)) {
                ...
                message = "Compra mínima para aplicar cupón es Q" + item.minimum_amount;
              }
